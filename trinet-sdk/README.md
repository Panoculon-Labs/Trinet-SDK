...
Device filter: the SDK ships an `xml/device_filter.xml` resource that
enumerates the USB vendor/product IDs the Trinet camera advertises across
its current variants. Reference it from your manifest as shown in
"Manifest setup" above; the active values are defined there.
